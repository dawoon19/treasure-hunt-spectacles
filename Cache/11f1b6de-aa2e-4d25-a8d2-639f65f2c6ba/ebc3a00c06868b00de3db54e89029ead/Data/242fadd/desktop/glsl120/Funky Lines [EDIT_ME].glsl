...
uniform float Port_RangeMinA_N069;
uniform float Port_RangeMaxA_N069;
uniform float Port_RangeMinB_N069;
uniform float Port_RangeMaxB_N069;
uniform float Port_Import_N073;
uniform float Port_Import_N074;
uniform vec2 Port_Direction_N084;
uniform float Port_Speed_N084;
uniform float Port_RangeMinB_N088;
uniform float Port_RangeMaxB_N088;
uniform float Port_RangeMinA_N089;
uniform float Port_RangeMaxA_N089;
uniform float Port_RangeMinB_N089;
uniform float Port_RangeMaxB_N089;
uniform float Port_Value3_N090;
uniform float Port_Value4_N090;
uniform vec4 Port_Import_N095;
uniform vec4 Port_Import_N096;
uniform float Port_Input0_N113;
...
uniform vec2 Port_Import_N107;
uniform float Port_Input1_N112;
uniform float Port_Input2_N112;
uniform float Port_Input2_N115;
uniform vec4 Port_Input0_N052;
uniform float Port_Input2_N032;
uniform float Port_TimeBias_N023;
uniform float Port_ValueA_N023;
uniform vec2 Port_Seed_N024;
uniform vec2 Port_Scale_N024;
uniform float Port_TimeTransition_N023;
uniform float Port_ValueB_N023;
uniform vec2 Port_Seed_N042;
uniform vec2 Port_Scale_N042;
uniform vec3 Port_Import_N010;
uniform float Port_Multiplier_N077;
uniform vec3 Port_Import_N012;
uniform vec2 Port_Scale_N015;
uniform vec2 Port_Scale_N029;
uniform vec2 Port_Scale_N063;
...
uniform vec4 Port_Import_N045;
uniform float Port_Import_N046;
uniform vec4 Port_Input1_N048;
...
uniform vec4 Port_Import_N036;
uniform float Port_Input1_N039;
uniform float Port_Value1_N040;
uniform float Port_Default_N041;
uniform float Port_Import_N099;
uniform float Port_Input1_N123;
uniform float Port_Value1_N124;
uniform float Port_Default_N125;
uniform float Port_Import_N126;
uniform float Port_Input1_N128;
uniform float Port_Value1_N129;
...
void Node23_Fluctuate(float TimeBias,float ValueA,float TimeA,float TimeTransition,float ValueB,float TimeB,out float Value,ssGlobals Globals)
{
float l9_0=((TimeA+TimeTransition)+TimeB)+9.9999997e-05;
Value=mix(ValueA,ValueB,smoothstep(TimeA/l9_0,1.0-(TimeB/l9_0),abs((mod((Globals.gTimeElapsed/l9_0)+TimeBias,1.0)*2.0)-1.0)));
}
float snoise(vec2 v)
{
...
void Node15_Noise_Simplex(vec2 Seed,vec2 Scale,out float Noise,ssGlobals Globals)
{
...
void Node63_Noise_Simplex(vec2 Seed,vec2 Scale,out float Noise,ssGlobals Globals)
{
Seed.x=floor(Seed.x*10000.0)*9.9999997e-05;
Seed.y=floor(Seed.y*10000.0)*9.9999997e-05;
Seed*=(Scale*0.5);
...
void Node97_Remap(vec3 ValueIn,out vec3 ValueOut,float RangeMinA,float RangeMaxA,float RangeMinB,float RangeMaxB,ssGlobals Globals)
{
ValueOut=(((ValueIn-vec3(RangeMinA))/vec3(RangeMaxA-RangeMinA))*(RangeMaxB-RangeMinB))+vec3(RangeMinB);
vec3 l9_0;
if (RangeMaxB>RangeMinB)
{
l9_0=clamp(ValueOut,vec3(RangeMinB),vec3(RangeMaxB));
}
else
{
l9_0=clamp(ValueOut,vec3(RangeMaxB),vec3(RangeMinB));
}
ValueOut=l9_0;
}
...
void LoopFunction_N2(float Index,float Ratio,inout vec4 Output,ssGlobals Globals)
{
Globals.Loop_Index_ID0=Index;
Globals.Loop_Ratio_ID0=Ratio;
float param_6;
Node23_Fluctuate(Port_TimeBias_N023,Port_ValueA_N023,floor(fract(sin(dot(floor(Port_Seed_N024*Port_Scale_N024),vec2(0.98979998,0.72329998)))*437.58545)*10000.0)*9.9999997e-05,Port_TimeTransition_N023,Port_ValueB_N023,floor(fract(sin(dot(floor(Port_Seed_N042*Port_Scale_N042),vec2(0.98979998,0.72329998)))*437.58545)*10000.0)*9.9999997e-05,param_6,Globals);
vec2 l9_0=vec2(Tweak_N21*param_6);
vec2 l9_1=vec2(Globals.gTimeElapsed*Port_Multiplier_N077);
float param_10;
Node15_Noise_Simplex((vec2(Globals.SurfacePosition_ObjectSpace.yz)*l9_0)+l9_1,Port_Scale_N015,param_10,Globals);
float param_14;
Node29_Noise_Simplex((vec2(Globals.SurfacePosition_ObjectSpace.zx)*l9_0)+l9_1,Port_Scale_N029,param_14,Globals);
float param_18;
Node63_Noise_Simplex((vec2(Globals.SurfacePosition_ObjectSpace.xy)*l9_0)+l9_1,Port_Scale_N063,param_18,Globals);
vec3 param_21;
Node97_Remap(cos((Globals.SurfacePosition_ObjectSpace+vec3((Globals.Loop_Ratio_ID0*Globals.Loop_Count_ID0)*Port_Input2_N032))+vec3((((param_10+param_14)+param_18)*Port_Input1_N065)*Port_Input1_N076)),param_21,Port_RangeMinA_N097,Port_RangeMaxA_N097,Port_RangeMinB_N097,Port_RangeMaxB_N097,Globals);
vec4 l9_2=sc_SampleTextureBiasOrLevel(baseTexDims.xy,baseTexLayout,baseTexGetStereoViewIndex(),(param_21+(param_21*vec3(Port_Input1_N078))).xy,(int(SC_USE_UV_TRANSFORM_baseTex)!=0),baseTexTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_baseTex,SC_SOFTWARE_WRAP_MODE_V_baseTex),(int(SC_USE_UV_MIN_MAX_baseTex)!=0),baseTexUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_baseTex)!=0),baseTexBorderColor,0.0,baseTex)*vec4(clamp(Port_Import_N046,0.0,5.0));
Output+=mix(Port_Input0_N052,mix(vec4(dot(l9_2,Port_Input1_N048)),l9_2,vec4(clamp(Port_Import_N049,0.0,2.0))),vec4(clamp(Port_Import_N051,0.0,2.0)));
}
...
vec2 l9_5=(((l9_4-vec2(Port_RangeMinA_N069))/vec2(Port_RangeMaxA_N069-Port_RangeMinA_N069))*(Port_RangeMaxB_N069-Port_RangeMinB_N069))+vec2(Port_RangeMinB_N069);
float l9_6=l9_5.x;
float l9_7=l9_5.y;
float l9_8=l9_1*Port_Speed_N084;
vec2 l9_9=((((vec2(atan(l9_7,l9_6))+(Port_Direction_N084*l9_8))-vec2(-3.1415927))/vec2(3.1415927-(-3.1415927)))*(Port_RangeMaxB_N088-Port_RangeMinB_N088))+vec2(Port_RangeMinB_N088);
float l9_10=(l9_6*l9_6)+(l9_7*l9_7);
float l9_11;
if (l9_10<=0.0)
{
l9_11=0.0;
...
float l9_12=l9_11-Port_RangeMinA_N089;
float l9_13=((l9_12/(Port_RangeMaxA_N089-Port_RangeMinA_N089))*(Port_RangeMaxB_N089-Port_RangeMinB_N089))+Port_RangeMinB_N089;
float l9_14=l9_9.x;
vec4 l9_15=vec4(l9_14,l9_13,Port_Value3_N090,0.0);
float l9_16=3.0*fract(l9_14+0.16666667);
float l9_17=2.0*fract(l9_16);
vec2 l9_18=l9_15.zz-((vec2(1.0,l9_17)-vec2(max(0.0,l9_17-1.0)))*(Port_Value3_N090*clamp(l9_13,0.0,1.0)));
...
vec2 l9_23=Port_Import_N107-Port_Import_N104;
float l9_24=smoothstep(Port_Input0_N113,Port_Input1_N113,clamp((dot(((((varPackedTex.xy-vec2(Port_RangeMinA_N101))/vec2(Port_RangeMaxA_N101-Port_RangeMinA_N101))*(Port_RangeMaxB_N101-Port_RangeMinB_N101))+vec2(Port_RangeMinB_N101))-Port_Import_N104,l9_23)/dot(l9_23,l9_23))+0.001,Port_Input1_N112+0.001,Port_Input2_N112+0.001)-0.001);
vec4 l9_25=vec4(0.0);
ssGlobals l9_26=ssGlobals(l9_1,l9_2,0.0,l9_4,varPackedTex.xy,0.0,0.0,(sc_ModelMatrixInverse*vec4(varPos,1.0)).xyz,5.0);
LoopFunction_N2(0.0,0.0,l9_25,l9_26);
LoopFunction_N2(1.0,0.25,l9_25,l9_26);
LoopFunction_N2(2.0,0.5,l9_25,l9_26);
LoopFunction_N2(3.0,0.75,l9_25,l9_26);
LoopFunction_N2(4.0,1.0,l9_25,l9_26);
vec4 l9_30=l9_25;
float l9_31;
#if (Tweak_N134)
{
float l9_32;
if ((float((opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).x)<=Port_Input1_N039)*1.0)!=0.0)
{
l9_32=Port_Value1_N040;
}
else
{
l9_32=opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).x;
}
l9_31=l9_32;
}
#else
{
l9_31=Port_Default_N041;
}
#endif
float l9_33;
#if (Tweak_N135)
{
float l9_34;
if ((float((opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).y)<=Port_Input1_N123)*1.0)!=0.0)
{
l9_34=Port_Value1_N124;
}
else
{
l9_34=opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).y;
}
l9_33=l9_34;
}
#else
{
l9_33=Port_Default_N125;
}
#endif
float l9_35;
#if (Tweak_N136)
{
float l9_36;
if ((float((opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).z)<=Port_Input1_N128)*1.0)!=0.0)
{
l9_36=Port_Value1_N129;
}
else
{
l9_36=opacity*sc_SampleTextureBiasOrLevel(SegmentationMaskDims.xy,SegmentationMaskLayout,SegmentationMaskGetStereoViewIndex(),varPackedTex.xy,(int(SC_USE_UV_TRANSFORM_SegmentationMask)!=0),SegmentationMaskTransform,ivec2(SC_SOFTWARE_WRAP_MODE_U_SegmentationMask,SC_SOFTWARE_WRAP_MODE_V_SegmentationMask),(int(SC_USE_UV_MIN_MAX_SegmentationMask)!=0),SegmentationMaskUvMinMax,(int(SC_USE_CLAMP_TO_BORDER_SegmentationMask)!=0),SegmentationMaskBorderColor,0.0,SegmentationMask).z;
}
l9_35=l9_36;
}
#else
{
l9_35=Port_Default_N131;
}
#endif
float l9_37=l9_31+l9_33;
float l9_38=l9_37+l9_35;
vec4 l9_39=vec4(mix(mix(vec4(1.0)-l9_22,l9_22,vec4((l9_24*(1.0-l9_24))*Port_Input2_N115)),l9_30/vec4(5.0),l9_22).xyz,l9_38);
...
{
l9_43=clamp(dot(l9_39.xyz,vec3(l9_38)),0.0,1.0);
}
#else
{
float l9_44;
...
{
l9_45=(1.0-dot(l9_39.xyz,vec3(0.33333001)))*l9_38;
}
#else
{
float l9_46;
...
{
l9_46=(1.0-clamp(dot(l9_39.xyz,vec3(1.0)),0.0,1.0))*l9_38;
}
#else
{
float l9_47;
...
{
l9_47=clamp(dot(l9_39.xyz,vec3(1.0)),0.0,1.0)*l9_38;
}
#else
{
float l9_48;
...
#if (sc_BlendMode_AddWithAlphaFactor)
{
l9_49=clamp(dot(l9_39.xyz,vec3(1.0)),0.0,1.0)*l9_38;
}
#else
{
float l9_50;
...
{
l9_50=dot(l9_39.xyz,vec3(0.33333001))*l9_38;
}
#else
{
float l9_51;
...
#else
{
vec2 l9_59=sc_ScreenCoordsGlobalToView(l9_3);
int l9_60;
...
#endif
vec3 l9_63=mix(l9_61.xyz,definedBlend(l9_61.xyz,l9_39.xyz).xyz,vec3(l9_38));
...
#if (sc_BlendMode_MultiplyOriginal)
{
l9_65=vec4(mix(vec3(1.0),l9_39.xyz,vec3(l9_38)),l9_38);
}
#else
{
vec4 l9_66;
