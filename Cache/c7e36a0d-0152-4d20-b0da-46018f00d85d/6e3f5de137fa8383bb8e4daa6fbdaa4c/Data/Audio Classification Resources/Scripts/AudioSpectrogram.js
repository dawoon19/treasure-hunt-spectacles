...
// @ui {"widget":"separator","showIf" : "advanced"}
// @input bool normalizeData = true {"showIf" : "advanced"}
// @ui {"widget":"separator","showIf" : "advanced"}
// @input bool createTexture = true {"showIf" : "advanced", "hint": "Please note that we are generating texture rotated by 90 degrees, you may need to create an input transformer when passing to the model and swap your x and y size here"}
...
function normalizeSpectrogramData() {
    ...
    if (max - min > EPS) {
        if (script.normalize) {
            oneValueResult[0] = std * (max - min); //1
            TensorMath.divTensors(data, shape, oneValueResult, VEC3_ONE, data);

            oneValueResult[0] = (mean / std + min) / (max - min); //0
...
function getSpectrogramTexture() {
    if (!script.createTexture) {
        print("Warning, Create texture checkbox is not enabled in Audio Spectrogram");
        return null;
    }
